...
function spawnEnemies()
  while #enemies < maxEnemies do
    ...
    if edge == 1 then
      x = camera.x - margin
      y = camera.y + math.random() * 600
    elseif edge == 2 then
      x = camera.x + 800 + margin
      y = camera.y + math.random() * 600
    elseif edge == 3 then
      x = camera.x + math.random() * 800
      y = camera.y - margin
    else
      x = camera.x + math.random() * 800
...
function love.draw()
  ...
  for x = startX, camera.x + 800, gridSize do
    local screenX = x - camera.x
    love.graphics.line(screenX, 0, screenX, 600)
  end
  for y = startY, camera.y + 600, gridSize do
    local screenY = y - camera.y
    love.graphics.line(0, screenY, 800, screenY)
  end
  ...
  love.graphics.setColor(1, 1, 1)
  love.graphics.rectangle("fill", 400 - player.size / 2, 300 - player.size / 2, player.size, player.size)
  ...
  if gameOver then
    ...
    love.graphics.print(gameOverText, 400 - textWidth / 2, 250)
    ...
    love.graphics.print(legendText, 400 - legendWidth / 2, 330)
  end
end
